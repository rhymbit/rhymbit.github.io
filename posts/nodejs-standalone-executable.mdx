...
Everyone remembers that Vercel’s pkg package ( [repo link](https://github.com/vercel/pkg) )? 
They’ve archived that repo and no longer maintain it, and that’s because :-
...
Now I'm Assuming you’ve written your NodeJs app and you’re ready to package it.
...
Doesn’t matter what sphagetti code you have in there, as long as it’s in src, you’re good to go 😊. 

We’re gonna use the almighty **Rollup.Js** to bundle our code and **create a single** `index.js` file that’ll hold everything.

### Adding Rollup to Project

Add these packages to your project.
Just a simple `npm install <package>` and you’re good to go.
...
You can install rollup globally using your fav package manager (me 💗 pnpm) 
and just run `rollup -c` in your terminal inside the project folder.

Now, Running this command should create one (just one ☝🏻) `index.js` inside the `dist` folder. 
...
- **Optional (for Windows only)**  
  <small>_This step is not worth it if you don’t have a license for the desktop app that you're creating._</small>  
...
- **Optional (for Windows only)**  
  <small>_Again, this step is not worth it if you don’t have a license for the desktop app that you're creating._</small>  

...
socialImage:
  filepathPublic: "/images/posts/electronjs-file-read-write.jpg"
  alt: "Easier File Read and Write in ElectronJS"
---
...
**A tip 👇🏻**  
An effective 💪🏻 and awesome ⭐ way to read files is through using a simple 
...
router.get('/:filename', (req, res) => {
  ...
  // 👇🏻 find this `fileSearch` algorithm in Related Posts dropdown
  fileSearch(
    ...
    filename, (err: Error, resultArr: string[]) => {
      if (err) {
        ...
        res.statusCode = 200;
        // 👇🏻 or 'application/json' or whatever based on data type
        res.contentType('text/plain');
        resultArr.length > 0 ? 
